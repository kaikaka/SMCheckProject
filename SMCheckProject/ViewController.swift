...
//        return ""
    ...
    @objc func cellDoubleClick() {
        if resultTb.selectedRow >= 0 && resultTb.selectedRow < unusedMethods.count {
            let aMethod = unusedMethods[resultTb.clickedRow]
    ...
    func cellOneClick() {
        if resultTb.selectedRow >= 0 && resultTb.selectedRow < unusedMethods.count {
            let aMethod = unusedMethods[resultTb.selectedRow]
            if let cFile = filesDic[aMethod.filePath] {
